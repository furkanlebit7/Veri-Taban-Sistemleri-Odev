        ...
        <div class="col-9">
          <div class="container mt-4 mb-4 p-3">
            <nav class="navbar navbar-light bg-light px-3">
              <form class="form-inline d-flex w-100 justify-content-between align-items-center" method="GET">
            ...
            <?php 
            if(isset($_GET["jobSearchSubmit"])){
            $jobInput=$_GET["jobInput"];

            $sorgu=mysqli_query($db,"SELECT job_advertisements.id,job_advertisements.description,job_advertisements.min_salary,job_advertisements.max_salary,job_advertisements.created_date,job_advertisements.job_feature,job_advertisements.job_type,cities.city_name,job_titles.title,employers.company_name FROM job_advertisements JOIN cities ON cities.id=job_advertisements.city_id JOIN employers ON employers.id=job_advertisements.employer_id JOIN job_titles ON job_titles.id=job_advertisements.job_title_id WHERE job_advertisements.is_active='1' AND job_titles.title LIKE '%$jobInput%' ORDER BY job_advertisements.created_date DESC");
                $colorCounter=0;
               while($sorguRow=mysqli_fetch_assoc($sorgu)){
                  ?>
                    <a href="./jobAdvertisementDetail.php?advertisementId=<?php echo$sorguRow["id"] ?>" style="text-decoration:none;" class="m-1">
                      <div class="card text-white <?php
                       ...
                       ?>" style="width: 18rem;">
                      <div class="card-header"><?php echo $sorguRow["company_name"] ?></div>
                      <div class="card-body">
                        <h5 class="card-title"><?php echo $sorguRow["title"] ?></h5>
                        <p class="card-title"><?php echo "( ".$sorguRow["city_name"]." )" ?></p>
                        <p class="card-title"><?php echo $sorguRow["job_feature"] ?></p>
                        <p class="card-title"><?php echo $sorguRow["job_type"] ?></p>
                        <p class="card-text"><?php echo $sorguRow["min_salary"]."₺ <---> ".$sorguRow["max_salary"]."₺" ?></p>
                      </div>
                      <div class="card-footer text-center">
                          <small class="text-white"><?php echo "📅 ".$sorguRow["created_date"]." 📅" ?></small>
                        </div>
                    </div>
                    </a>
                  <?php
